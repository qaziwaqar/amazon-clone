...
**Status:** TODO · **Depends on:** 00 · **Budget:** ~40m
...
- [ ] Tailwind, fonts, and the base colour tokens render in both light and dark

## Tasks

- [ ] `create-next-app` — TypeScript, Tailwind, App Router, ESLint, `src/`, import alias `@/*`
- [ ] Strip boilerplate: default page, unused SVGs, starter CSS
- [ ] Design tokens in `globals.css` — Amazon-adjacent palette (squid-ink `#131921`, nav `#232F3E`, accent `#FEBD69`, link `#007185`, price red `#B12704`) as CSS variables, never hard-coded per component
- [ ] `src/lib/` skeleton: `db.ts`, `utils.ts` (`cn`, `formatPrice`), `constants.ts`
- [ ] Placeholder `/` that renders the token palette, so the deploy is visibly *something*
- [ ] `.env.example` with every variable the app will need, no real values
